...
function Cursor({ children, className }: CursorProps) {
  ...
  const styleIdRef = React.useRef<string>(`cursor-style-${Math.random().toString(36).slice(2)}`);

  React.useEffect(() => {
    const parentElement = containerRef.current?.parentElement;
    if (!parentElement) return;

    // Add unique attribute to parent for CSS targeting
    const attrName = `data-cursor-${styleIdRef.current}`;

    if (isActive) {
      parentElement.setAttribute(attrName, "true");

      // Create style element to hide all cursors within the parent
      let styleEl = document.getElementById(styleIdRef.current);
      if (!styleEl) {
    ...
    return () => {
      parentElement.removeAttribute(attrName);
