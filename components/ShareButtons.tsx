...
export default function ShareButtons({
  ...
  /**
   * Add UTM parameters to URL for tracking share sources
   */
  ...
  const handleWebShare = async () => {
    if (navigator.share) {
      try {
        const shareUrl = addUTM(url, 'web_share')
  ...
  const handleFacebookShare = () => {
    const facebookUrl = addUTM(url, 'facebook')
  ...
  const handleLinkedInShare = () => {
    const linkedinUrl = addUTM(url, 'linkedin')
    ...
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
      ...
      {canShare && (
        <button
          ...
          className="group relative rounded-full bg-primary/10 p-2 text-primary transition-all hover:bg-primary/20 hover:scale-110"
